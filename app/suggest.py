...
def ML(filename):
    ...
    token = util.\
        prompt_for_user_token(username, scope, client_id=client_id,
                              client_secret=client_secret,
                              redirect_uri='http://localhost:8888/callback')
    spo = spotipy.Spotify(auth=token)
    playlist_name = "".join(random.choice(string.ascii_letters)
                            for i in range(12))
    ...
    spo.user_playlist_add_tracks(username, playlist_id,
                                 rec_tracks['uri'].values.tolist())
